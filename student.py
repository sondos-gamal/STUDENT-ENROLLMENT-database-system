...
def refreshTable():
    ...
    for array in read():
        item_counter += 1
        iid = str(item_counter)
        
        my_tree.insert(parent='', index='end', iid=iid, text="", values=(array), tag="orow")
...
def read():
    ...
    # Now you can execute SQL statements using the cursor object
    cursor.execute('SELECT * from  student')
    results = cursor.fetchall()

    # Remember to close the cursor and connection when you're finished
    cursor.close()
    conn.close()
